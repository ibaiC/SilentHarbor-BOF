...
// Props - https://github.com/trustedsec/CS-Remote-OPs-BOF/blob/main/src/common/bofdefs.h
...
static const BOF_GUID WINTRUST_ACTION_GENERIC_VERIFY_V2 =
    { 0xaac56b, 0xcd44, 0x11d0, { 0x8c, 0xc2, 0x00, 0xc0, 0x4f, 0xc2, 0x95, 0xee } };

typedef struct _BOF_WINTRUST_FILE_INFO {
    DWORD cbStruct;
    LPCWSTR pcwszFilePath;
    HANDLE hFile;
    void *pgKnownSubject;
} BOF_WINTRUST_FILE_INFO;

typedef struct _BOF_WINTRUST_DATA {
    DWORD cbStruct;
    ...
    DWORD dwUnionChoice;
    union {
        struct _BOF_WINTRUST_FILE_INFO *pFile;
    };
    DWORD dwStateAction;
...
WINBASEAPI DWORD WINAPI KERNEL32$GetProcessId(HANDLE);
...
WINBASEAPI HMODULE WINAPI KERNEL32$GetModuleHandle (LPCSTR lpModuleName);
WINBASEAPI WINBOOL WINAPI KERNEL32$DuplicateHandle (HANDLE hSourceProcessHandle,HANDLE hSourceHandle, HANDLE hTargetProcessHandle, LPHANDLE lpTargetHandle, DWORD dwDesiredAccess, BOOL bInheritHandle, DWORD dwOptions);
...
//PSAPI
DECLSPEC_IMPORT WINBOOL WINAPI PSAPI$EnumProcesses(DWORD *lpidProcess,DWORD cb, DWORD *lpcbNeeded);
DECLSPEC_IMPORT WINBOOL WINAPI PSAPI$EnumProcessModules(HANDLE hProcess,HMODULE *lphModule,DWORD cb,LPDWORD lpcbNeeded);
DECLSPEC_IMPORT DWORD WINAPI PSAPI$GetModuleBaseNameW(HANDLE hProcess,HMODULE hModule,LPWSTR lpBaseName,DWORD nSize);
DECLSPEC_IMPORT DWORD WINAPI PSAPI$GetModuleFileNameExA(HANDLE hProcess, HMODULE hModule, LPSTR lpFilename, DWORD nSize);

//WINTRUST
DECLSPEC_IMPORT LONG WINAPI WINTRUST$WinVerifyTrust(HWND hwnd, GUID *pgActionID, LPVOID pWVTData);
...
#define KERNEL32$GetModuleHandle GetModuleHandle
...
//WINTRUST
#define WINTRUST$WinVerifyTrust WinVerifyTrust
